...
export default function IncidentTypeManageForm() {
  const { incidentTypeEnumList, setIncidentTypeEnumList } = useProfileDataContext();
  const [newType, setNewType] = useState('');

  const handleAddType = (e: React.FormEvent) => {
    e.preventDefault();
    if (newType.trim()) {
      if (!incidentTypeEnumList.includes(newType.trim())) {
        setIncidentTypeEnumList([...incidentTypeEnumList, newType.trim()]);
        setNewType('');
      }
    }
  };
  ...
  const handleRemoveType = (typeToRemove: string) => {
    setIncidentTypeEnumList(incidentTypeEnumList.filter(type => type !== typeToRemove));
  };
